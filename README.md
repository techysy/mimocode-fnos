...
- **双架构**：x86_64 与 arm64 均已构建并校验

## 安装协议授权

安装时会显示**使用协议与授权声明**，条款按性质归为**三项**，逐项勾选后即可继续：

| 类别 | 涵盖内容 |
| :--- | :--- |
| **一、来源与授权** | 非官方第三方适配声明；引擎版权归 Xiaomi Corporation（MIT 协议）；本包保留原始版权声明 |
| **二、风险与担保** | 按「现状」提供、无任何担保；使用风险自负（数据丢失、服务中断、模型费用、密钥泄露）；需访问 NAS 文件系统并以独立用户执行命令 |
| **三、网络与暴露** | 默认监听 19281 且允许终端写入；建议仅在内网使用，勿直接暴露公网 |

每项均提供 `tip` 展开完整条款原文，勾选框内为条款摘要。

该协议由 [`wizard/install`](wizard/install) 定义，使用 fnOS 安装向导的 `checkbox` 必选类型实现（`required: true`，任一项未勾选则无法安装）。
